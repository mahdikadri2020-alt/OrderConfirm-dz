...
export default function FeaturesSection({ onGoToApp }) {
  ...
  return (
    <section id="features" className="w-full py-12 sm:py-16 md:py-24 px-4 sm:px-8 md:px-12 lg:px-20 bg-secondary/30 border-t border-border/50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          {features.map((item, idx) => (
            <div
              key={idx}
              className="bg-background rounded-2xl p-5 sm:p-6 md:p-8 border border-border/80 shadow-xs hover:shadow-md hover:border-accent/30 transition-all group"
            >
              <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-secondary flex items-center justify-center mb-4 sm:mb-5 group-hover:scale-110 transition-transform">
                {item.icon}
              </div>
              <h3 className="text-base sm:text-lg font-heading font-bold text-foreground mb-2">{item.title}</h3>
              <p className="text-xs sm:text-sm font-body text-muted-foreground leading-relaxed">{item.description}</p>
            </div>
          ))}
        </div>

        {/* Highlight Banner */}
        <div className="mt-10 sm:mt-16 bg-primary text-primary-foreground rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl text-center md:text-left">
          <div className="space-y-2 max-w-xl">
            <h3 className="font-heading font-bold text-xl sm:text-2xl md:text-3xl leading-tight">
              Prêt à doubler votre taux de livraison en Algérie ?
            </h3>
            <p className="text-primary-foreground/80 font-body text-xs sm:text-sm md:text-base">
              Essayez OrderConfirm gratuitement avec 100 confirmations offertes à l'inscription. Sans carte bancaire.
            </p>
          </div>
          <button
            onClick={onGoToApp}
            className="w-full sm:w-auto shrink-0 bg-white text-foreground hover:bg-slate-100 px-6 py-3.5 rounded-full text-xs sm:text-sm font-heading font-semibold transition-all flex items-center justify-center gap-2 shadow-lg"
          >
