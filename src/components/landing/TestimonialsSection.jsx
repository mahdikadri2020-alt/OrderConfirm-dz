...
export default function TestimonialsSection() {
  ...
  return (
    <section id="testimonials" className="w-full py-20 px-6 md:px-12 lg:px-20 bg-secondary/30 border-t border-border/50">
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-display text-4xl md:text-5xl text-foreground mt-4 tracking-tight">
            Validé par plus de <span className="italic">200+ boutiques</span> en Algérie
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((rev, idx) => (
            <div
              key={idx}
              className="bg-background rounded-3xl p-8 border border-border/80 shadow-sm flex flex-col justify-between"
            >
              <div>
              ...
              <div className="mt-6 flex items-center gap-3 pt-4 border-t border-border/50">
                <div className="h-10 w-10 rounded-full bg-accent/20 text-accent font-bold flex items-center justify-center text-sm">
                  {rev.name.substring(0, 2).toUpperCase()}
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-foreground">{rev.name}</h4>
